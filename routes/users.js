...
function convertMilliseconds(x) {
  ...
  if (diffInMillis < 0) {
    ...
    diffInMillis = 4 * 3.6e6 - diffInMillis;

    x.timeToCancel = prettyMilliseconds(diffInMillis, {
      verbose: true,
      compact: true,
    });
  } else {
    x.timeToStart = prettyMilliseconds(diffInMillis, {
      verbose: true,
      compact: true,
    });
  }
...
router.get("/view-car-details/:id", userData, async (req, res, next) => {
  ...
  res.render("users/view-car-details", {
    reviews,
    userData: res.userData,
    carsData,
    user: req.session.user,
    userNotVerified: req.session.userNotVerified,
  });
...
router.get("/login", redirectToHome, function (req, res, next) {
  res.render("users/user-login", {
    usernotExists: req.session.usernotExists,
...
router.get("/upcomming-trips/:id", userData, (req, res) => {
  bookingHelpers.upCommingBookings(req.params.id).then((bookingData) => {
    ...
    res.render("users/upcomming-trips", {
      ...
      user: req.session.user,
      bookingData,
      paymentSuccessful: req.session.paymentSuccessful,
    });
...
router.get("/profile", async (req, res) => {
  ...
  let walletBalance = await bookingHelpers.getTotalRefundAmount(
...
router.post("/update-profile", (req, res) => {
  uploadMultiple(req, res, async (err) => {
    ...
    if (req.files.profilepic) {
      let profileLocation = await uploadToS3.uploadLicenceProfile(
        req.files.profilepic[0].filename
      );
      req.body.profileImage = profileLocation;
    }
    if (req.files.licenceimg) {
      let LicenceLocation = await uploadToS3.uploadLicenceProfile(
        req.files.licenceimg[0].filename
      );
...
router.get("/apply-coupon/:coupon", async (req, res) => {
  let couponApplied = await userHelpers.CheckCouponApplied(
    req.session.user._id,
    req.params.coupon
  );
  if (!couponApplied) {
...
router.post("/is-order-complete", (req, res) => {
  ...
  var expectedSignature = crypto
    .createHmac("sha256", "JBvcyTFdq5gZtqjNkOxwOrgn")
...
router.get("/add-to-wishlist/:carId", async (req, res) => {
  if (req.session.user) {
    let response = await userHelpers.addToWishlist(
      req.session.user._id,
      req.params.carId
    );
    res.json(response);
...
router.get("/wishlist", redirectToLogin, userData, async (req, res) => {
  let carsData = await userHelpers.getCarDetailsInWishlist(
    req.session.user._id
  );
...
router.get("/booking-history", redirectToLogin, userData, async (req, res) => {
  let tripData = await bookingHelpers.getCompletedTripsByUser(
    req.session.user._id
  );
  ...
  res.render("users/booking-history", {
    ...
    userData: res.userData,
    user: req.session.user,
    reviewPosted: req.session.reviewPosted,
  });
...
router.get(
  ...
  userData,
  async (req, res) => {
    let tripData = await bookingHelpers.getCancelledBookingByUser(
      req.session.user._id
    );
    for (x of tripData) {
      x.from = moment(x.from).format("lll");
    ...
    res.render("users/cancelled-bookings", {
      tripData,
      userData: res.userData,
      user: req.session.user,
    });
  ...
  paypal.payment.execute(
    ...
    function (error, payment) {
      if (error) {
        ...
        bookingHelpers
          .newBooking(bookingDetails, req.session.coupon)
          .then(() => {
            bookingDetails = {};
            req.session.paymentSuccessful = true;
            res.redirect("/upcomming-trips/" + req.session.user._id);
